...
func copyPrivateKey(keystore string, key bccsp.Key) error {
	id := hex.EncodeToString(key.SKI())
	privateKeyBytes, err := ioutil.ReadFile(filepath.Join(keystore, id+"_sk"))
	if err != nil {
		return err
	}
	err = ioutil.WriteFile(filepath.Join(keystore, "privateKeyName_sk"), privateKeyBytes, 0644)
	if err != nil {
		return err
	}
	return nil
	// return os.Rename(filepath.Join(keystore, id+"_sk"), "privateKeyName_sk")
}
...
func GenerateLocalMSPForAdmin(baseDir, name string, sans []string, signCA *ca.CA,
	...
	*/
	...
	err = copyPrivateKey(keystore, priv)
	if err != nil {
		return err
	}
